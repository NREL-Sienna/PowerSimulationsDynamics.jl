...
Mach3_benchmark = OneDOneQMachine(
    0.0, #R
    1.3125, #Xd
    1.2578, #Xq
    0.1813, #Xd_p
...
KundurShaft = SingleMass(
    3.525, #H given in example 3.5 in Kundur
    2.0,
) #D: Damping is not given, so I assume the value 2.0

Shaft2_benchmark = SingleMass(
    3.01, #H (M = 6.02 -> H = M/2)
    0.0,
) #D

Shaft3_benchmark = SingleMass(
    3.01, #H (M = 6.02 -> H = M/2)
    0.0,
) #D
...
Gen4AVRTG = PSY.DynamicGenerator(
    1, #number
    :TestGen, #name
    nodes_OMIB[2], #bus
    1.0, # ω_ref
...
GenKundur = PSY.DynamicGenerator(
    1, #number
    :KundurGen, #name
    nodes_kundur[1],
    1.0, #ω_ref
...
GenFullKundur = PSY.DynamicGenerator(
    1, #number
    :KundurGen, #name
    nodes_kundur[1],
    1.0, #ω_ref
...
Gen2_benchmark_tg = PSY.DynamicGenerator(
    1, #number
    :Gen1, #name
    nodes_benchmark[2],
    1.0, #ω_ref
...
Gen3_benchmark = PSY.DynamicGenerator(
    2, #number
    :Gen2, #name
    nodes_benchmark[3],
    1.0, #ω_ref
...
Gen2_benchmark_fixedavr = PSY.DynamicGenerator(
    1, #number
    :Gen1, #name
    nodes_benchmark[2],
    1.0, #ω_ref
...
Gen3_benchmark_fixedavr = PSY.DynamicGenerator(
    2, #number
    :Gen2, #name
    nodes_benchmark[3],
    1.0, #ω_ref
...
Gen_bench = PSY.DynamicGenerator(
    1, #number
    :Gen1, #name
    nodes_bench[2],
    1.0, #ω_ref
...
inf_gen_kundur = StaticSource(1, :InfBus, nodes_kundur[2], 1.01, 0.0, 0.000005)

inf_gen_benchmark = StaticSource(
    1, #number
    :InfBus, #name
...
#include("src/LITS.jl")
OMIB_dyn_line =
    PSY.System(nodes_OMIB, branch_OMIB, [Gen1AVR], [inf_gen], 100.0, 60.0, Dynbranch_OMIB)

OMIBA = PSY.System(nodes_OMIB, branch_OMIB, [Gen5AVR], [inf_gen], 100.0, 60.0)
OMIB_Kundur =
    PSY.System(nodes_kundur, branch_kundur, [GenKundur], [inf_gen_kundur], 100.0, 60.0)
OMIB_FullKundur =
    PSY.System(nodes_kundur, branch_kundur, [GenFullKundur], [inf_gen_kundur], 100.0, 60.0)
TwoBus_Benchmark = PSY.System(
    nodes_bench, #buses
...
ThreeBus_Benchmark_tg = PSY.System(
    nodes_benchmark, #buses
    branch_benchmark, #branches
...
ThreeBus_Benchmark_fixedavr = PSY.System(
    nodes_benchmark, #buses
    branch_benchmark, #branches
