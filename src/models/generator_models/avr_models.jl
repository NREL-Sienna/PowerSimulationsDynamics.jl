function mdl_avr_ode!(
    ...
    get_inner_vars(dyn_data)[Vf_var] = PSY.get_Vf(PSY.get_avr(dyn_data))
...
function mdl_avr_ode!(
    ...
    local_ix = get_local_state_ix(dyn_data, PSY.AVRSimple)
    ...
    #Get Parameters
    Kv = PSY.get_Kv(PSY.get_avr(dyn_data))

    #Compute ODEs
    output_ode[local_ix[1]] = Kv * (V_ref - V_th)

    #Update inner_vars
    get_inner_vars(dyn_data)[Vf_var] = Vf
...
function mdl_avr_ode!(
    ...
    #Obtain references
    V0_ref = PSY.get_ext(dyn_data)[CONTROL_REFS][V_ref_index]

    #Obtain indices for component w/r to device
    local_ix = get_local_state_ix(dyn_data, PSY.AVRTypeI)
    ...
    Vs = get_inner_vars(dyn_data)[V_pss_var]
...
function mdl_avr_ode!(
    ...
    #Obtain indices for component w/r to device
    local_ix = get_local_state_ix(dyn_data, PSY.AVRTypeII)
...
function mdl_avr_ode!(
    ...
    #Obtain indices for component w/r to device
    local_ix = get_local_state_ix(dyn_data, typeof(avr))
    ...
    Vs = get_inner_vars(dyn_data)[V_pss_var]
    Xad_Ifd = get_inner_vars(dyn_data)[Xad_Ifd_var]
