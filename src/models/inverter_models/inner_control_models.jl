...
function mdl_inner_ode!(
    ...
    inner_vars::AbstractArray{<:ACCEPTED_REAL_TYPES},
    dynamic_device::DynamicWrapper{
        PSY.DynamicInverter{C, O, PSY.CurrentLimitiedVoltageModeControl, DC, P, F},
    },
) where {
    C <: PSY.Converter,
    O <: PSY.OuterControl,
    ...
    #Obtain external states inputs for component
    external_ix = get_input_port_ix(dynamic_device, PSY.CurrentLimitiedVoltageModeControl)
    Ir_filter = device_states[external_ix[1]]
    Ii_filter = device_states[external_ix[2]]
    Ir_cnv = device_states[external_ix[3]]
    Ii_cnv = device_states[external_ix[4]]
    ...
    #Obtain indices for component w/r to device
    local_ix = get_local_state_ix(dynamic_device, PSY.CurrentLimitiedVoltageModeControl)
    ...
    #TODO - add in the current limiting! 
