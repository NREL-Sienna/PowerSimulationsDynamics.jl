function network_model(inputs::SimulationInputs, cache::Cache, V::AbstractArray{T}) where T <: Real
    ...
    for n in UnitRange{Int}(1, bus_count)
        ...
        for i in SparseArrays.nzrange(ybus, n + bus_count)
            I_balance[n] -= ybus_vals[i]*V[rows_vals[i]]
        end
        for i in SparseArrays.nzrange(ybus, n)
            I_balance[n + bus_count] -= ybus_vals[i]*V[rows_vals[i]]
        end
    end
