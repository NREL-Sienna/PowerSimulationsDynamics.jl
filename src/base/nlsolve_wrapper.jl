...
function refine_initial_condition!(
    ...
    if sim.status == SIMULATION_INITIALIZED
        CRC.@ignore_derivatives @info "Simulation already initialized. Refinement not executed"
        return
    end
    converged = false
    ...
    if !converged || (sum(residual) > MINIMAL_ACCEPTABLE_NLSOLVE_F_TOLERANCE)
        ...
        CRC.@ignore_derivatives @warn("Initialization didn't found a solution to desired tolerances.\\
