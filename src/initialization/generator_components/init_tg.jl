...
function initialize_tg!(
    ...
    if !NLsolve.converged(sol)
        CRC.@ignore_derivatives @warn("Initialization of Turbine Governor $(PSY.get_name(static)) failed")
    else
        sol_x0 = sol.zero
        #Update Control Refs
...
function initialize_tg!(
    ...
    sol = NLsolve.nlsolve(f!, x0; ftol = STRICT_NLSOLVE_F_TOLERANCE)
    if !NLsolve.converged(sol)
        CRC.@ignore_derivatives @error("Initialization of Turbine Governor $(PSY.get_name(static)) failed")
    else
        sol_x0 = sol.zero
        #Update Control Refs
        PSY.set_P_ref!(tg, sol_x0[1])
