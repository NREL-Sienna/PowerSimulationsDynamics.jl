...
function initialize_avr!(
    ...
    if !NLsolve.converged(sol)
        CRC.@ignore_derivatives @warn("Initialization of AVR in $(PSY.get_name(static)) failed")
    else
        sol_x0 = sol.zero
        #Update V_ref
...
function initialize_avr!(
    ...
    if !NLsolve.converged(sol)
        ...
        if (y_ll2 > Va_max) || (y_ll2 < Va_min)
            CRC.@ignore_derivatives @error("Regulator Voltage V_r = $(y_ll2) outside the limits")
        end
    end
    return
end
...
function initialize_avr!(
    ...
    if (V_r20 > Vr_max) || (V_r20 < Vr_min)
        CRC.@ignore_derivatives @error("Regulator Voltage V_R = $(V_r20) outside the limits")
    end
