...
function initialize_dynamic_device!(
    ...
    if !NLsolve.converged(sol)
        CRC.@ignore_derivatives @warn("Initialization in Ind. Motor $(PSY.get_name(device)) failed")
    else
        sol_x0 = sol.zero
...
function initialize_dynamic_device!(
    ...
    sol = NLsolve.nlsolve(f!, x0; ftol = STRICT_NLSOLVE_F_TOLERANCE)
    if !NLsolve.converged(sol)
        CRC.@ignore_derivatives @warn("Initialization in Active Load $(PSY.get_name(device)) failed")
    else
        sol_x0 = sol.zero
